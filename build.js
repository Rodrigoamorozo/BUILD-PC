...
const MOCK_PECAS = [
  { id: 'cpu-1', categoria: 'cpu', nome: 'Ryzen 5 7600', marca: 'AMD', preco: 1299, socket: 'AM5', potencia_w: 65 },
  { id: 'cpu-2', categoria: 'cpu', nome: 'Core i5-13400F', marca: 'Intel', preco: 1099, socket: 'LGA1700', potencia_w: 65 },
  { id: 'cpu-3', categoria: 'cpu', nome: 'Ryzen 7 7800X3D', marca: 'AMD', preco: 2599, socket: 'AM5', potencia_w: 120 },
  ...
  { id: 'gpu-1', categoria: 'placa-video', nome: 'RTX 4060', marca: 'NVIDIA', preco: 2199, potencia_w: 115 },
  { id: 'gpu-2', categoria: 'placa-video', nome: 'RTX 4070', marca: 'NVIDIA', preco: 3699, potencia_w: 200 },
  { id: 'gpu-3', categoria: 'placa-video', nome: 'RX 7600',  marca: 'AMD',    preco: 1899, potencia_w: 165 },
  { id: 'gpu-4', categoria: 'placa-video', nome: 'RTX 5090',  marca: 'NVIDIA', preco: 1899, potencia_w: 580 },
  { id: 'gpu-5', categoria: 'placa-video', nome: 'RX 580',  marca: 'AMD', preco: 1899, potencia_w: 135 },
  { id: 'gpu-6', categoria: 'placa-video', nome: 'RTX 3050 TI',  marca: 'NVIDIA', preco: 1899, potencia_w: 35 },
...
function checarCompatibilidade() {
  const avisos = [];
  const { cpu, ['placa-mae']: placaMae, memoria, fonte } = build;
...
function renderPecas() {
  ...
  pecasDaCategoria.forEach(peca => {
    const selecionada = build[categoriaAtiva]?.id === peca.id;
    const card = document.createElement('button');
    card.className = `text-left border rounded-xl p-5 transition-colors ${
      selecionada
...
function renderResumo() {
  ...
  const escolhidas = CATEGORIAS.map(cat => build[cat.id]).filter(Boolean);
  if (!escolhidas.length) {
    lista.innerHTML = `<li class="text-muted-light dark:text-muted">Nenhuma peça escolhida ainda.</li>`;
  } else {
    escolhidas.forEach(peca => {
      const li = document.createElement('li');
      li.className = 'flex justify-between gap-3';
      li.innerHTML = `<span class="text-muted-light dark:text-muted">${peca.nome}</span><span>R$ ${peca.preco.toLocaleString('pt-BR')}</span>`;
      lista.appendChild(li);
    });
  }

  const total = escolhidas.reduce((soma, p) => soma + p.preco, 0);
